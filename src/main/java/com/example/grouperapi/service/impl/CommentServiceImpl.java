...
public class CommentServiceImpl implements CommentService {
    ...
    private void postComment2() {
        PostComment topLevelReply = createTopComment(1L, "user6", """
                ...
                There are some optimizations possible. For example, a file which is what we typically refer to as a file (say, a text file stored on a disk) can be memory mapped, and simply shown by piping the memory to stdout. But that will break on some things, and is therefore not advisable to use in the generic case of cat, which has to work with everything.
        ...
        Reply reply2 = createReply("user6", """
                ...
                All I mean by naive is that it's akin to cat, and not highly optimized for when there are more efficient ways of accessing media and such. That doesn't necessarily mean it's simple, as it has to handle a lot of corner cases.
                """);
    ...
    @Transactional
    void postComment1() {
        PostComment topLevelReply1 = createTopComment(1L, "user2", "It depends on the exact implementation, but most of them either use a fixed size buffer or a syscall to have the kernel transfer data directly from the input to the output.");
